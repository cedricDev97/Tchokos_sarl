...
function renderOrdersAdminTable(list){
  ...
  tbody.innerHTML = filtered.map(o=>{
    ...
    const btnHtml = `<div class="hero-row" style="gap:6px; flex-wrap:wrap">${viewBtn}${advanceBtn}${cancelBtn}${failBtn}</div>`;
  ...
  if(filtered.length === 0){
    tbody.innerHTML = `<tr><td colspan="8" class="muted">Aucune commande pour ce filtre.</td></tr>`;
  }
}
...
async function renderDashboard(){
  try{
    ...
    if(!aRes.ok || !aData.ok) throw new Error(aData.error || "Analytics API error");

    adminStatsCache = sData;
    adminOrdersCache = oData.results || [];
    adminAnalyticsCache = aData;
